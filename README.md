...
./rollback-openclaw.sh 20260717-004530 --skip-node  # roll back gateway only; leave node alone
```
...
- **Node rollbacks downgrade the npm package** from the version captured by `upgrade-openclaw.sh` before the upgrade (saved as `openclaw-node-<TIMESTAMP>.version` under `~/openclaw-backups`). If that file is missing — for example when the upgrade was performed manually — the node side is left untouched and the gateway rollback proceeds.
